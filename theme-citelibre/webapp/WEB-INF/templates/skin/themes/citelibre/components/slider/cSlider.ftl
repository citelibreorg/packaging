...
<@cSection id=id class='themecitelibre-slider-wrapper' params='aria-roledescription="slide" aria-label="${sliderLabel!}" ${params}'>
	<@cRow id='${id}-theme-slider' class='themecitelibre-slider ${class!}' params='aria-live="off"'>
	<#if slides?size gt 0>
		<#assign slideIdx=1 />
			...
			<@cBlock class='d-flex justify-content-end pt-4 pb-md-5 mb-md-5'>
				...
				<#if buttonNext !=''>
					<@cBtn label='' class='${buttonClass} ms-sm next-slide' type='button' params='tabindex="0" aria-controls="${id}-theme-slider" aria-label="${buttonNext!} ${slideLabel}"'> 
						<@citelibreIcon name='arrow-right' />
					</@cBtn>
				</#if>
				</@cBlock>
			</@cBlock>
...
document.addEventListener('DOMContentLoaded', function (){
	const _slides = $('#${id} .themecitelibre-slider'),
