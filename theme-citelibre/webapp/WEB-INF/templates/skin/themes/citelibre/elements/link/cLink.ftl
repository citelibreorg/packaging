...
-->
<#macro cLink href label title='' titleClass='' nestedPos='after' target='' showTarget=false class='' id='' params='' deprecated...>
<@deprecatedWarning args=deprecated />
<a href="${href!}" <#if id!=''> id="${id!}"</#if> <#if class !='' >class="${class!}"</#if><#if title!=''> title="${title!}<#if target='_blank'> #i18n{themecitelibre.newWindowLink}</#if>"</#if><#if target!=''> target="${target}"</#if><#if params!=''> ${params}</#if> > 
<#if nestedPos!='after'><#nested></#if>
<#if label!=''><span class="link-label<#if titleClass!=''> ${titleClass!}</#if>">${label!}</span></#if> 
<#if nestedPos='after'> <#nested></#if>
<#if target='_blank' && label=''><span class="visually-hidden visually-hidden-focusable"><#if title!=''>${title!}<#else>${label}</#if> #i18n{themecitelibre.newWindowLink}</span></#if>
