...
@param - pmLabel - string - Default '#i18n{themecitelibre.labelPasswordStrength} #i18n{themecitelibre.labelPasswordNoPasswordTyped}' Label asssocié ua message sur la force du mot de passe
...
@param - helpMsg - string - Default '#i18n{themecitelibre.labelPasswordHelp}', Message d'aide pour l'input                                   
...
@sample : 
 ...
 -->
<#macro cInputPassword name label='#i18n{themecitelibre.labelPassword}' btnShowPassword=true passwordMeter=false pmLabel='#i18n{themecitelibre.labelPasswordStrength} #i18n{themecitelibre.labelPasswordNoPasswordTyped}' pmUrl='' pmConfirmFieldId='' placeholder='' autocomplete='' class='' id='' size='' value='' required=true disabled=false maxlength=100 helpMsg='#i18n{themecitelibre.labelPasswordHelp}' errorMsg='' params='' deprecated...>
<@deprecatedWarning args=deprecated />
<#local passId><#if id !=''>${id!}<#else>${name!}</#if></#local>
<#local passLabel><#if pmLabel !=''>${pmLabel!}<#else>#i18n{themecitelibre.labelPasswordStrength} #i18n{themecitelibre.labelPasswordNoPasswordTyped}</#if></#local>
<#local passClass>form-control-input pwd<#if class!=''> ${class!}</#if><#if size!=''> form-control-input-${size!}</#if><#if errorMsg!=''> is-invalid</#if></#local>
...
<@cInputGroup class='password'>
    ...
    <#if btnShowPassword>
    <@cBtn class='secondary toggle-password' type='button' label='' params='data-bs-toggle="#${passId}" aria-pressed="false" title="#i18n{themecitelibre.labelPasswordShow}" tabindex="0"'>
         <@citelibreIcon name='eye-off' class='main-info-color' />
    </@cBtn>
    </#if>
    </@cInput>
</@cInputGroup>
...
function togglePasswordIcon( field, show=false ){
  ...
  if( show && show !=undefined ){
      if( input.attr("type") == "password") {
          ...
          $(this).attr('title','#i18n{themecitelibre.labelPasswordHide}');
      }
  } else {
    if( input.attr("type") == "password") {
        input.attr("type", "text");
        btnToggle.attr('aria-pressed','true');
        btnToggle.attr('title','#i18n{themecitelibre.labelPasswordHide}');
    } else {
        input.attr("type", "password");
        btnToggle.attr('aria-pressed','false');
        btnToggle.attr('title','#i18n{themecitelibre.labelPasswordShow}');
    }
  }
}
</#if>
document.addEventListener( "DOMContentLoaded", function(e){
      ...
      <#if pmUrl !=''>
        $.ajax({
            ...
            success: function ( data) {
                ...
                showIndicator( '${passId}', '${pmLabel}', '#i18n{themecitelibre.labelPasswordStrength}'  )
            },
    ...
    passwordStrength( '${passId}', '${pmLabel}', '#i18n{themecitelibre.labelPasswordStrength}' )
<#if !isTogglePasswordLoaded?? || !isTogglePasswordLoaded>
