...
-->  
<#macro cTable caption='' captionClass='visually-hidden' summary='' breakpoint='' themed=true collapsible=true class='' id='' params='' deprecated...>
<@deprecatedWarning args=deprecated />
<#if id=''><!-- ATTENTION : Risque de duplication d'id ! --></#if>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const rows = document.querySelectorAll('table#${id} tbody tr'), tbId='${id}';
    rows.forEach(function(row, index) {
        const cells = row.querySelectorAll('td');
        let accordionContent = '';

        cells.forEach(function(cell, i) {
            const headerText = document.querySelector(<#noparse>`table#${tbId} thead th:nth-child(${i + 1})`</#noparse>).textContent;
