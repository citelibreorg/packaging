...
@param - labelMore - string - optional - Libellé du bouton pour afficher le contenu (par défaut: i18n("themecitelibre.labelShowMore"))
...
<#macro cStepper steps=steps haspicto=false hasidx=false title=false showMore=false labelMore=i18n("themecitelibre.labelShowMore") a11StatusMsg='' class='' id='' params='' deprecated...>
	                ...
	                <#else>
	                    <div class="idx" data-idx="${step?index + 1}" aria-hidden="true">
	                    	<#if step.status?? && (step.status == 'done')>
	                    		<@citelibreIcon name='check' title='' class='main-color' params='aria-label="Etape Validée"' />
	                    	<#else>
